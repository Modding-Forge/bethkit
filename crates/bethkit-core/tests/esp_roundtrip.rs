...
//!
//! All `.esp`, `.esm`, and `.esl` files found in `tests/testdata/` (workspace
//! root) are tested automatically. Place any Skyrim SE plugin file there to
//! add it to the suite.
//!
//! Run with:
//! ```text
//! cargo test --test esp_roundtrip -p bethkit-core -- --nocapture
//! ```

use std::path::{Path, PathBuf};

use bethkit_core::{
    FormId, GameContext, Plugin, PluginKind, PluginPatcher, RecordFlags, Signature,
};

/// Returns all `.esp` / `.esm` / `.esl` files in `tests/testdata/`.
///
/// Navigates from `CARGO_MANIFEST_DIR` up to the workspace root and into
/// `tests/testdata/`. Returns an empty list if the directory does not exist
/// so that CI without real plugin files still passes.
...
fn all_plugins_parse_without_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !failures.is_empty() {
        ...
        panic!(
            "{} plugin(s) failed to parse — see stderr for details",
            failures.len()
        );
    }
...
fn all_plugins_have_accessible_groups() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for path in &paths {
        // when
        let Ok(plugin) = open_plugin(path) else {
            continue;
        };
        // then — just ensure the call does not panic
...
/// `.esm` files must not be detected as the Update-only variant.
#[test]
...
fn all_subrecords_parse_without_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !failures.is_empty() {
        eprintln!(
            "{} of {total_records} records failed subrecord parsing:",
            failures.len()
        );
    ...
    eprintln!("OK: {total_records} records parsed");
    Ok(())
}
...
fn edid_subrecords_are_valid_utf8() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for path in &paths {
        ...
        for group in plugin.groups() {
            for record in group.records_recursive() {
                let Ok(Some(edid_sr)) = record.get(Signature::EDID) else {
                    continue;
                };
                edid_count += 1;
                // then
...
/// Compressed records must decompress without error.
#[test]
...
fn no_op_patcher_is_byte_identical_for_all_plugins() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let paths = collect_testdata();
    if paths.is_empty() {
        eprintln!("[skip] no plugin files in tests/testdata/");
        return Ok(());
    }

    let mut checked = 0usize;
    let mut mismatches: Vec<(PathBuf, usize, usize)> = Vec::new();

    for path in &paths {
        // when
        let original: Vec<u8> = std::fs::read(path)?;
        let plugin = open_plugin(path).map_err(|e| -> Box<dyn std::error::Error> { e.into() })?;
        let patcher = PluginPatcher::new(&plugin);
        let mut rewritten: Vec<u8> = Vec::with_capacity(original.len());
        patcher.write_to(&mut rewritten)?;

        // then
        if rewritten != original {
            mismatches.push((path.clone(), original.len(), rewritten.len()));
        }
        checked += 1;
    }
    ...
    if !mismatches.is_empty() {
        ...
        panic!(
            "{} of {checked} plugins did not round-trip",
            mismatches.len()
        );
    }

    eprintln!("Round-trip verified for {checked} plugins");
    Ok(())
}
